...
export default function ProductDetailPage() {
  ...
  const [product, setProduct] = useState<typeof mockVisualProducts[0] | null>(null);
  const [selectedColor, setSelectedColor] = useState<string | null>(null);

  const colorMap: Record<string, string> = {
    blanco: "#ffffff", negro: "#000000", rojo: "#ff0000", azul: "#0000ff", beige: "#f5f5dc",
    rosa: "#ffc0cb", gris: "#808080", verde: "#008000", grisclaro: "#d3d3d3", verdeoscuro: "#006400",
    azulmarino: "#000080", verdeoliva: "#808000", grisoscuro: "#505050", rojoclaro: "#ff6666",
    verdemilitar: "#4b5320", azulclaro: "#add8e6", azuloscuro: "#00008b", azullima: "#c4e538",
    arena: "#f0e68c", marron: "#8b4513", azulpetroleo: "#004b49", celeste: "#87ceeb", grisperla: "#c0c0c0"
  };
  ...
  if (!product) return <p className="p-6 text-white">Producto no encontrado.</p>;

  const selectedColorIndex = product.colors.indexOf(selectedColor || '');
  const imageKeys = ['imagecolor1', 'imagecolor2'];
  const selectedImage =
    (product[imageKeys[selectedColorIndex] as keyof typeof product] as string | undefined) ||
    product.imagecolor1 ||
    ...
    <div className="p-6 max-w-5xl mx-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mt-4">
        ...
        <div className="text-white">
          ...
          <div className="mb-4">
            <span className="text-yellow-400 text-xl font-semibold">${product.price}</span>
            <span className="text-sm text-gray-400 ml-4">Stock: {product.stock}</span>
          </div>
            ...
            <div className="flex gap-3">
              {product.colors.map((color) => (
                <button
                  ...
                  className={`w-7 h-7 rounded-full border-2 transition ${selectedColor === color ? 'border-yellow-400 scale-110' : 'border-white'
                    }`}
